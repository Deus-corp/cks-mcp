...
![Tests](https://img.shields.io/badge/tests-958%20passed%2C%206%20skipped-brightgreen)
...
that gives LLMs a **canonical knowledge backbone**. It exposes **69
...
4. Claude automatically picks the right tool from the 69 available — validation, evolution, branching, merging, source verification, contradiction detection, semantic search, subgraph queries, sandboxing, and more.
...
3. Save the file and fully restart Claude Desktop (Cmd+Q, then reopen).
   After restart, a connector icon will appear – `cks-mcp` with 69 tools is ready to use.
...
69 tools, grouped by function. Full reference with parameters and
...
958+ tests: 952 passing, 6 skipped (require Postgres or optional providers not configured in a default environment).
