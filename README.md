...
that gives LLMs a **canonical knowledge backbone**. It exposes **41
...
4. Claude automatically picks the right tool from the 41 available — validation, evolution, branching, merging, source verification, contradiction detection, semantic search, subgraph queries, sandboxing, and more.
...
3. Save the file and fully restart Claude Desktop (Cmd+Q, then reopen).
   After restart, a connector icon will appear – `cks-mcp` with 41 tools is ready to use.
...
41 tools, grouped by function. Full reference with parameters and
