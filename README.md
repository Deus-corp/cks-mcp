...
that gives LLMs a **canonical knowledge backbone**. It exposes **49
...
4. Claude automatically picks the right tool from the 49 available — validation, evolution, branching, merging, source verification, contradiction detection, semantic search, subgraph queries, sandboxing, and more.
...
3. Save the file and fully restart Claude Desktop (Cmd+Q, then reopen).
   After restart, a connector icon will appear – `cks-mcp` with 49 tools is ready to use.
...
49 tools, grouped by function. Full reference with parameters and
