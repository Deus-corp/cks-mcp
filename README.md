...
![Tests](https://img.shields.io/badge/tests-774%20passed-brightgreen)
...
that gives LLMs a **canonical knowledge backbone**. It exposes **61
...
4. Claude automatically picks the right tool from the 61 available — validation, evolution, branching, merging, source verification, contradiction detection, semantic search, subgraph queries, sandboxing, and more.
...
3. Save the file and fully restart Claude Desktop (Cmd+Q, then reopen).
   After restart, a connector icon will appear – `cks-mcp` with 61 tools is ready to use.
...
61 tools, grouped by function. Full reference with parameters and
...
renewal interval, default 61). See `cks_mcp/fork_resolution_agent.py` for the
