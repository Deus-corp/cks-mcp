...
![Tests](https://img.shields.io/badge/tests-574%20passed-brightgreen)
...
that gives LLMs a **canonical knowledge backbone**. It exposes **47
...
4. Claude automatically picks the right tool from the 47 available — validation, evolution, branching, merging, source verification, contradiction detection, semantic search, subgraph queries, sandboxing, and more.
...
3. Save the file and fully restart Claude Desktop (Cmd+Q, then reopen).
   After restart, a connector icon will appear – `cks-mcp` with 47 tools is ready to use.
...
47 tools, grouped by function. Full reference with parameters and
...
| Export & Observability | `export_knowledge`, `export_session`, `get_metrics`, `export_storage`, `import_storage`, `migrate_storage` |
| Memory & Persistence | `register_graph`, `get_graph`, `list_graphs`, `search_graphs`, `check_graph_freshness` |
