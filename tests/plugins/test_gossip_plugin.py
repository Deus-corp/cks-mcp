...
# ---------------------------------------------------------------------------
# CRDT store integration (ADR-013 Stage 1)
# ---------------------------------------------------------------------------

async def test_setup_creates_crdt_store_for_sqlite_backend(
    plugin: GossipPlugin, mock_runtime: MagicMock, mock_config: MagicMock
) -> None:
    """When runtime.storage wraps a SQLiteStorage, a SQLiteCRDTStore
    is created and passed through to setup_gossip."""
    from unittest.mock import MagicMock

    fake_conn = MagicMock()
    mock_wrapped = MagicMock()
    mock_wrapped._conn = fake_conn
    mock_runtime.storage = MagicMock()
    mock_runtime.storage.wrapped = mock_wrapped
    ...
    with (
        ...
        await plugin.setup(mock_runtime, mock_config)

    mock_setup_gossip.assert_called_once()
    _, kwargs = mock_setup_gossip.call_args
    assert "crdt_store" in kwargs
    assert kwargs["crdt_store"] is not None