"""Unit tests for cks_mcp.agents.critic_agent.critic_agent."""
...
from cks_mcp.agents.critic_agent.critic_agent import (
    CriticAgentSettings,
    LLMCircuitBreaker,
    Resolution,
    _process_one,
...
async def test_gossip_conflict_merges_cleanly(mock_runtime, monkeypatch):
    ...
    monkeypatch.setattr("cks_mcp.agents.critic_agent.critic_agent.merge_branch", _fake_merge_branch)
...
async def test_inference_conflict_stale_premise_only_is_actually_resolved(
    ...
    monkeypatch.setattr("cks_mcp.agents.critic_agent.critic_agent.arbitrate_inference_conflict", _fake_arbitrate)
...
async def test_inference_conflict_auto_resolves_and_commits(mock_runtime, monkeypatch):
    ...
    monkeypatch.setattr(
        "cks_mcp.agents.critic_agent.critic_agent.arbitrate_inference_conflict", _fake_arbitrate
    )
...
async def test_provenance_conflict_refreshes_and_commits(mock_runtime, monkeypatch):
    ...
    monkeypatch.setattr(
        "cks_mcp.agents.critic_agent.critic_agent.refresh_verification", _fake_refresh_verification
    )
...
async def test_temporal_conflict_bumps_valid_until_and_commits(mock_runtime, monkeypatch):
    ...
    monkeypatch.setattr(
        "cks_mcp.agents.critic_agent.critic_agent._resolve_temporal_conflict_tool",
...
async def test_contradiction_conflict_resolves_and_commits(mock_runtime, monkeypatch):
    ...
    monkeypatch.setattr(
        "cks_mcp.agents.critic_agent.critic_agent._resolve_contradiction_tool",
...
async def test_process_one_completes_on_success(mock_runtime, monkeypatch):
    ...
    monkeypatch.setitem(
        __import__("cks_mcp.agents.critic_agent.critic_agent", fromlist=["_RESOLVERS"])._RESOLVERS,
        "gossip_conflict",
        _fake_resolver,
    )
...
async def test_lease_lost_during_resolution_is_not_completed_or_failed(mock_runtime):
    ...
    import cks_mcp.agents.critic_agent.critic_agent as critic_agent_module
...
async def test_breaker_resets_on_success(mock_runtime, monkeypatch):
    ...
    monkeypatch.setattr("cks_mcp.agents.critic_agent.critic_agent.arbitrate_inference_conflict", _fake_success)
...
async def test_breaker_does_not_trip_on_non_llm_error(mock_runtime, monkeypatch):
    ...
    monkeypatch.setattr("cks_mcp.agents.critic_agent.critic_agent.arbitrate_inference_conflict", _fake_structural_error)
