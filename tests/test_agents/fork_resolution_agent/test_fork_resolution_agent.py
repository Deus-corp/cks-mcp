...
class TestLcaResolutionIdempotency:
    ...
    async def test_skips_evolve_when_resolution_object_already_recorded(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "cks_mcp.agents.fork_resolution_agent.fork_resolution_agent._find_owning_session_id",
            lambda *_a, **_k: "s1",
        )
        evolve_mock = AsyncMock()
        monkeypatch.setattr("cks_mcp.agents.fork_resolution_agent.fork_resolution_agent.evolve_knowledge", evolve_mock)
        monkeypatch.setattr(
            "cks_mcp.agents.fork_resolution_agent.fork_resolution_agent.resolve_with_lca",
...
class TestCausalityWinner:
    ...
    def test_dominating_candidate_wins(self):
        ...
        with patch(
            "cks_mcp.agents.fork_resolution_agent.fork_resolution_agent.causality_check",
...
class TestSelectWinner:
    def test_newest_timestamp_wins_when_causality_fails(self):
        ...
        with patch(
            "cks_mcp.agents.fork_resolution_agent.fork_resolution_agent._find_causally_newest", return_value=None
        ):
...
class TestResolveFork:
    ...
    async def test_no_candidate_objects_still_exist(self):
        ...
        with patch(
            "cks_mcp.agents.fork_resolution_agent.fork_resolution_agent._crdt_store_for", return_value=store
        ):
            task = {
                "payload": {
                    "pointer_key": "key1",
    ...
    async def test_no_crdt_store_available(self):
        with patch("cks_mcp.agents.fork_resolution_agent.fork_resolution_agent._crdt_store_for", return_value=None):
            task = {
                "payload": {
                    "pointer_key": "key1",
                    "conflicting_object_ids": ["a", "b"],
