...
- [ ] **LLM-assisted gossip conflict resolution:** `resolve_gossip_conflict` tool — mirrors `arbitrate_inference_conflict`'s three-path shape for structural merge conflicts, closing the asymmetry where inference conflicts had LLM arbitration but gossip conflicts required hand-rolled `resolutions`. Once shipped, the Critic Agent can auto-resolve structural gossip conflicts instead of dead-lettering them.
