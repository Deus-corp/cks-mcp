...
`cks-mcp` exposes **65 tools** over the Model Context Protocol. Every tool
...
| [Memory & Persistence](export-and-audit.md) | `register_graph`, `get_graph`, `clone_graph`, `list_graphs`, `search_graphs`, `check_graph_freshness`, `check_component_versions`, `update_registered_graph`, `explain_graph`, `check_graph_health` | Save and reuse Knowledge Graphs, check component freshness, update outdated graphs, generate reports, compute health scores |
