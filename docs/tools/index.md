...
`cks-mcp` exposes **62 tools** over the Model Context Protocol. Every tool
...
| [LLM & AI] (ai-assisted.md)| `ai_chat`, `construct_knowledge`, `suggest_evolution`, `ingest_document`, `request_enrichment`, `get_llm_status` | AI chat with tool calling, knowledge construction from text/URL, LLM provider status |
