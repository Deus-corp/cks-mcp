...
| [Export & Observability](export-and-audit.md) | `export_knowledge`, `export_session`, `get_metrics`, `export_storage`, `import_storage`, `migrate_storage` | Get data out, back up, migrate between backends, and see how the server is performing |
| [Memory & Persistence](export-and-audit.md) | `register_graph`, `get_graph`, `list_graphs`, `search_graphs`, `check_graph_freshness` | Save and reuse Knowledge Graphs across conversations by registering them under memorable names, browse/search a public gallery of them, and check whether one has gone stale |
