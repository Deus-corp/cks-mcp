...
``cks_mcp.agents.critic_agent.critic_agent`` and ``cks_mcp.agents.enrichment_agent.enrichment_agent`` (both built on
the shared ``cks_mcp.agents.agent_loop`` helpers) -- but run as its own,
dedicated console script/process rather than folded into the Critic
Agent's own ``crdt_fork`` handling.

.. note::
   ``cks_mcp.agents.critic_agent.critic_agent`` already claims and resolves ``crdt_fork``
...
from cks_mcp.agents.agent_loop import (
    LivenessReporter,
    Resolution,
    run_resolver_with_heartbeat,
)
from cks_mcp.agents.fork_resolution_agent.lca_arbiter import resolve_with_lca
...
# cks_mcp.agents.critic_agent.critic_agent's own re-export -- this module's tests (and any
...
class ForkResolutionAgentSettings:
    ...
    # Whether to try the topology-aware LCA arbiter (cks_mcp.agents.fork_resolution_agent.lca_arbiter)
...
def _crdt_store_for(runtime: Runtime) -> Any | None:
    ...
    ``cks_mcp.gossip._build_crdt_store``/``cks_mcp.agents.critic_agent.critic_agent._crdt_store_for``
...
async def run_fork_agent(
    *,
    ...
    liveness = LivenessReporter(
        runtime, "fork_resolution", settings.liveness_interval, stop_event=stop
    )
