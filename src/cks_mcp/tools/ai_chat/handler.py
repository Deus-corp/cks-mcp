...
Implements a bounded agentic loop over the Anthropic Messages API's
tool-use feature: the LLM is offered every registered tool except a
small denylist of server-management tools, may call them freely across
up to _MAX_ITERATIONS round-trips, and every session-shaped argument it
...
from cks_mcp.llm_providers import call_anthropic_with_tools
